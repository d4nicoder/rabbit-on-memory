import {
  ...
  IQueueBinding,
  IQueueInternal,
} from "./interfaces/IQueueBinding.js";
...
export default class RabbitOnMemory {
      ...
      list[i].callback(message)
        .then(() => {
          if (this.configuration.debug) {
            console.log(
              `Successful executed event on queue ${list[i].queue}, route ${
                list[i].bindRoute
              }`,
            );
          }
  ...
  private async processQueuesFanout(message: IMessage): Promise<void> {
    const queues = Array.from(this.routes.values()).reduce(
      (accum: IQueueInternal[], queues: IQueueInternal[]) => {
        return accum.concat(queues.filter((queue) =>
          queue.exchange === message.fields.exchange
        ));
      },
      [],
    );
  ...
  private async processQueuesDirect(
    route: string,
    message: IMessage,
  ): Promise<void> {
    const queues = Array.from(this.routes.entries()).reduce(
      (accum: IQueueInternal[], queues: [string, IQueueInternal[]]) => {
        if (queues[0] === route) {
          accum = accum.concat(queues[1].filter((queue) =>
            queue.exchange === message.fields.exchange
          ));
        }
        return accum;
  ...
  private async processQueuesTopic(
    route: string,
    message: IMessage,
  ): Promise<void> {
      ...
      (accum: IQueueInternal[], queues: [string, IQueueInternal[]]) => {
        ...
        for (let i = 0; i < messageSplitedRoute.length; i++) {
          if (messageSplitedRoute[i] === queuesSplitedRoute[i]) {
            ...
            return accum.concat(queues[1].filter((queue) =>
              queue.exchange === message.fields.exchange
            ));
          } else if (queuesSplitedRoute[i] === "*") {
            // Match this segment
  ...
  private deleteExpired(): void {
    this.routes.forEach((queues, route) => {
      const filtered = queues.filter((queue) => {
        if (
          !queue.options || !queue.options.expires || !queue.options.expireTime
        ) {
          return true;
        }
  ...
  public unbindQueue(exchange: string, queue: string): void {
    ...
    this.routes.forEach((queues, route) => {
      const filtered = queues.filter((q) =>
        q.queue !== queue || q.exchange !== exchange
      );
      this.routes.set(route, filtered);
    });
  }

  public async publishRoute(
    exchange: string,
    routingKey: string,
    content: unknown,
    ...
    const exchangeType: IExchangeType | undefined = this.exchanges.get(
      exchange,
    );
    ...
    if (exchangeType === "fanout") {
      ...
      error.message =
        `Exchange type ${exchangeType} for ${exchange} not supported`;
  ...
  public setConfig(options: IConfigOptions) {
    if (typeof options.syncMode === "boolean") {
      ...
      if (this.configuration.debug) {
        console.log(
          `Events sync mode ${
            this.configuration.syncMode ? "active" : "disabled"
          }`,
        );
      }
    }
    ...
    if (typeof options.propagateExceptionsOnSyncMode === "boolean") {
      this.configuration.propagateExceptionsOnSyncMode =
        options.propagateExceptionsOnSyncMode;
    }
  }
}
